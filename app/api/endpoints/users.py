...
@router.get(
    ...
    description="Retrieve multiple users. " + SUPERUSER_PRIVILEGE_DESC,
...
@router.get(
    ...
    description="Retrieve current user information. " + BASIC_USER_DESC,
    status_code=status.HTTP_200_OK,
)
...
@router.get(
    ...
    description="Retrieve any user information. " + SUPERUSER_PRIVILEGE_DESC,
    status_code=status.HTTP_200_OK,
)
def read_user(
    *,
...
@router.post(
    ...
    description="Create a user without the need to be logged in. " + PUBLIC_DESC,
)
...
@router.post(
    ...
    response_model=schemas.User,
    status_code=status.HTTP_201_CREATED,
    description="Create a new superuser as a superuser. " + SUPERUSER_PRIVILEGE_DESC,
)
...
@router.patch(
    cur_user_endpoint,
    response_model=schemas.User,
    description="Update currently logged in user. " + BASIC_USER_DESC,
)
...
@router.patch(
    cur_user_password_endpoint,
    response_model=schemas.User,
    description="Update current user password. " + BASIC_USER_DESC,
)
...
@router.patch(
    user_endpoint,
    response_model=schemas.User,
    description="Update any user with superuser. " + SUPERUSER_PRIVILEGE_DESC,
)
def update_user(
    *,
    db: Session = Depends(deps.get_db),
