...
@router.get(base_endpoint, response_model=List[schemas.User], description=SUPERUSER_PRIVILEGE_DESC)
def read_users(
...
@router.post(open_endpoint, response_model=schemas.User, status_code=status.HTTP_201_CREATED)
...
@router.patch(user_endpoint, response_model=schemas.User, description=SUPERUSER_PRIVILEGE_DESC)
def update_user(
	*,
	db: Session = Depends(deps.get_db),
	user_id: int,
