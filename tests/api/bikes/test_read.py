...
def test_read_bikes(client: TestClient, bikes_with_model: List[schemas.BikeWithModel]):
	response = client.get("/api/v1/bikes")
	assert response.status_code == 200
	bikes_test_list = response.json()
	assert len(bikes_test_list) == 3

def test_read_bikes_model_param(client: TestClient, bikes_with_model: List[schemas.BikeWithModel]):
	params = {
		"model": "test bike model 1"
	}
...
def test_read_bikes_offset_limit_param(client: TestClient, bikes_with_model: List[schemas.BikeWithModel]):
	params = {
		"limit": 1
	}
	response = client.get("/api/v1/bikes", params=params)
...
def test_read_bike(client: TestClient, bikes_with_model: List[schemas.BikeWithModel]):
